// D:\polyglot_connect\src\data\shared_content.ts

// Import the types. Path is from src/data/ to src/js/types/
import type { SharedContent, TutorImage } from '../js/types/global.d.ts';

console.log('data/shared_content.ts: loading...');

const sharedContentData: SharedContent = {
    tutorImages: [
        {
            file: "market_scene.jpg",
...
window.polyglotSharedContent = sharedContentData;

console.log(
    "data/shared_content.ts loaded.",
    (window.polyglotSharedContent?.tutorImages?.length || 0), "tutor images defined,",
    (window.polyglotSharedContent?.homepageTips?.length || 0), "homepage tips."
);

console.log("data/shared_content.ts loaded. ... "); // Your existing log
document.dispatchEvent(new CustomEvent('polyglotSharedContentReady'));
console.log("data/shared_content.ts: 'polyglotSharedContentReady' event dispatched.");